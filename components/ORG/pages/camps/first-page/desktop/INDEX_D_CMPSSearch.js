...
export const INDEX_D_CMPSSearch = ({ positionInArray, isSelected = false, componentName }) => {
  ...
  useEffect(() => {
    if (!isSelected) {
      ...
      setHowMuchDisplay(DATA_PCMPS_D.length)
    }
  }, [isSelected])
  ...
  const handleMoveToThirdPage = (e, theData, subCategoryArrPosition, resourceArrPosition, titleSubCategory) => {
    let getDataLeft = DATA_DAY_D_CardLeft[resourceArrPosition]
    let getDataRight = DATA_DAY_D_CardRight[resourceArrPosition]
    ...
    if (!failed) {
      ...
      let getDetailName = DATA_DAY_D_CardRight[resourceArrPosition][DATA_ORG_KeyNamesForCards_D.THIRD_PAGE_DATA][DATA_ORG_KeyNamesForCards_D.FOLDER_NAME]
