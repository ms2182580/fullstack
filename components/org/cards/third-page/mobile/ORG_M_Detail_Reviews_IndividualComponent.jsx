...
export const ORG_M_Detail_Reviews_IndividualComponent = ({
  getReviews,
  isModal = false,
}) => {
  const allUserNames = useMemo(
    () => ORG_ReviewsUsersName(getReviews.length),
    []
  )
  ...
  return (
    <ORG_M_Detail_Reviews_IndividualComponentWrapper isModal={isModal}>
      {getReviews.map((xReview, index) => {
        let shouldDisplaySomeFinalAdditionalText = index % adittionalData.length

        const modifiedArray = adittionalData[0]
          .split(" ")
          .map((item, index) => {
            if (index < 2) {
              ...
              return (
                <span className="LAST_TWO" key={index}>
                  {item}
                </span>
              )
            }
            ...
            >
              ...
              <div>
                <span>
                  {index === 0
                    ? modifiedArray
                    : adittionalData[shouldDisplaySomeFinalAdditionalText]}
                </span>
                {index === 2 && (
                  <span tabIndex={0} data-text="Verified Review">
                    <Image src={ORG_D_Detail_Review} alt="verified icon" />
                  </span>
                )}
              </div>
            </footer>
