...
export const ORG_ST_About = (name, lastName) => {
  ...
  const capitalizeFullName = capitalizeWords(fullName)
  const capitalizeName = capitalizeWords(name, true)

  let firstChange = pickedOne.replace("__FULLNAMEHERE__", capitalizeFullName)
  let secondChange = firstChange.replaceAll("__NAMEHERE__", capitalizeName)
