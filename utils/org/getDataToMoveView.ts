...
export const getDataToMoveView = ({
  categoryPosition,
  subcategoryPosition,
  resourcePosition = null,
}: GetDataToMoveView_Type) => {
  let folder =
    DATA_ORG_D[categoryPosition].acronym ||
    "pat" /* This «pat» is a fallback if acronym doesn't exist on the object */
  let subFolder = DATA_ORG_CheckPaths_Results_D[folder][subcategoryPosition]

  const theActualFilter =
    AllFilters[`${folder}_filters`][`filtersUI_${folder.toUpperCase()}`]

  const acronymSubcategory =
    DATA_ORG_CheckPaths_Results_D[folder].at(-2)[subFolder]
  ...
  const subcategory =
    theActualData[`DATA_${folder.toUpperCase()}_D`][subcategoryPosition]
  ...
  const subcategorySpecificData =
    subcategory.slice(1)[0][SPECIFIC_DATA_KEY.SPECIFIC_DATA_KEY] ?? null
